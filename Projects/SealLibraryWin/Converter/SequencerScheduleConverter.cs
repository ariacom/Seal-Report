...
{
    public class SequencerScheduleConverter : StringConverter
    {
        ...
        public override StandardValuesCollection GetStandardValues(ITypeDescriptorContext context)
        {
            ...
            if (report != null)
            {
                var list = report.Repository.Configuration.GetSequencerReportSchedules();
                choices = (from s in list select s.Name).ToList();
                if (context.PropertyDescriptor.Name != "ScheduleGUID")
                {
                    choices.Insert(0, "");
                }
            }
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destType)
        {
            if (context != null)
            {
                ...
                {
                    var list = report.Repository.Configuration.GetSequencerReportSchedules();
                    ReportSchedule schedule = list.FirstOrDefault(i => i.GUID == value.ToString());
                    if (schedule != null) return schedule.Name;
                }
            }
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            ...
            {
                var list = report.Repository.Configuration.GetSequencerReportSchedules();
                ReportSchedule schedule = list.FirstOrDefault(i => i.Name == value.ToString());
                if (schedule != null) return schedule.GUID;
            }

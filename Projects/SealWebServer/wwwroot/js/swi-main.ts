...
class SWIMain {
    ...
    private loginSuccess(data: any) {
        ...
        if ((window as any).SWIAIPanel) (window as any).SWIAIPanel.applyProfile(data);
        _main._reportPath = "";
        _main._folder = null;
    ...
    private setClipboard(cut: boolean, message: string) {
        _main._clipboard = [];
        $(".report-checkbox:checked").each(function (key, value) {
    ...
    private pasteClipboard(message: string, op: (source: string, destination: string, cb: () => void) => void) {
        if (_main._clipboard.length == 0) return;
        SWIUtil.ShowModal($waitDialog);
    ...
    private buildReportsTable(data: any) {
        ...
        for (var i = 0; i < data.files.length; i++) {
            ...
            if (_main._canEdit) $tr.append($("<td class='d-none d-sm-table-cell'>").append($("<input>").addClass("report-checkbox").prop("type", "checkbox").data("path", file.path)));
    ...
    public load(data: any[]): boolean {
        ...
        var build = function (folders: any[], parentUl: HTMLElement) {
            for (var i = 0; i < folders.length; i++) {
                ...
                if (f.right == 4 && !f.files) canEdit = true;
